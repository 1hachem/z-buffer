...
###############################################################
#                                                             #
# MATH202: Math̩matiques pour le num̩rique II                #
#                                                             #
# Mini biblioth̬que pour manipuler des images au format PPM.  #
#                                                             #
# Auteur: Xavier Proven̤al, Pierre Hyvernat                  #
#                                                             #
###############################################################
...
class Image:
    ...
    def __init__(self, *args):
        if len(args) == 0:
            raise ValueError("N̩cessite un nom de fichier ou des dimensions")
    ...
    def getPixel(self, x, y):
        if not (0 <= x < self.width):
            print("[ERREUR] Num̩ro de colonne (x={}) invalide.".format(x))
            assert(False)
        if not (0 <= y < self.height):
            print("[ERREUR] Num̩ro de ligne (y={}) invalide.".format(y))
            assert(False)
    ...
    def setPixel(self, x, y, couleur):
        if not (0 <= x < self.width):
            raise ValueError("[ERREUR] Num̩ro de colonne (x={}) invalide.".format(x))
        if not (0 <= y < self.height):
            raise ValueError("[ERREUR] Num̩ro de ligne (y={}) invalide.".format(y))
        k = 3*x + 3*self.width*y
